...
class DistFeature(nn.Module):
    def __init__(self, input_size, output_size):
        super(DistFeature, self).__init__()
        self.bin_input_size = input_size // config.N_bins
        self.stat_dim = self.bin_input_size - 1
        # Using an embedding for fit type with fixed embedding dim=3.
        self.fit_embed_dim = 16
        self.bin_output_size = self.stat_dim + self.fit_embed_dim
        self.fit_embed = nn.Embedding(3, self.fit_embed_dim)
        # Add position embedding for bins
        self.pos_embed = nn.Embedding(config.N_bins, self.bin_output_size)
        
        self.bin_linear = nn.Sequential(
            nn.Linear(self.bin_output_size, self.bin_output_size),
            nn.LayerNorm(self.bin_output_size),
            nn.GELU()
        )
        self.concat_linear = nn.Sequential(
            nn.Linear(self.bin_output_size * config.N_bins, output_size),
            nn.GELU()
        )
        
    def forward(self, x):
        # x: tensor with last dimension equal to input_size.
        n_batch, n_node, n_column = x.shape[:3]
        x_view = x.view(-1, self.bin_input_size)
        stat_part, fit_type = torch.split(x_view, [self.stat_dim, 1], dim=-1)
        fit_type = fit_type.long().squeeze(-1)
        fit_embed = self.fit_embed(fit_type)
        bin_embed = self.bin_linear(torch.cat((stat_part, fit_embed), dim = -1))
        
        # Reshape to separate bins
        bin_embed = bin_embed.view(-1, config.N_bins, self.bin_output_size)
        
        # Create position indices for each bin
        pos_indices = torch.arange(config.N_bins, device=x.device)
        pos_embed = self.pos_embed(pos_indices)
        bin_embed = bin_embed + pos_embed.unsqueeze(0)
        bin_embed = bin_embed.view(-1, config.N_bins * self.bin_output_size)
        concat_embed = self.concat_linear(bin_embed)
        if len(x.shape) > 3:
            concat_embed = concat_embed.view(n_batch, MAXNODE, n_column, -1)
        else:
            concat_embed = concat_embed.view(n_batch, -1)
        return concat_embed


class Column(nn.Module):
    """Module to process column-related features dynamically."""
    def __init__(self):
        ...
        if 'semantic' in self.feature_dict:
            ...
            self.feature_modules['semantic'] = nn.Sequential(
...
class Table(nn.Module):
    def __init__(self,columnNet = None):
        ...
        if 'tableEmbed' in self.feature_dict:
            ...
            self.feature_modules['tableEmbed'] = nn.Sequential(
                nn.Linear(input_dim, out_dim),
                nn.GELU()
            )
            self.final_input_dim += out_dim
        ...
        if 'columnIdx' in self.feature_dict:
            input_dim, out_dim = self.feature_dict['columnIdx'] # Get configured dims
            if columnNet is not None: # Only if a valid module is passed
                self.feature_modules['columnIdx'] = columnNet
                self.final_input_dim += out_dim # Add configured out_dim
            # else: if columnNet is None, this feature part is skipped
    ...
    def get_all_table_embedding(self):
        """Return embeddings for all tables."""
        self.eval()
        with torch.no_grad():
            embeddings = []
            for key, module in self.feature_modules.items():
                feat = self.table_feature[key]
                # Handle 'columnIdx' specifically if it might involve averaging or different processing
                if key == 'columnIdx' and 'columnIdx' in self.feature_modules: # check module exists
                    embeddings.append(module(feat).mean(dim=-2) if hasattr(module(feat), 'mean') else module(feat))
                elif key != 'columnIdx':
                    embeddings.append(module(feat))
            
            if not embeddings:
                 # Similar to forward, handle empty embeddings if necessary
                 # For this edit, assume it proceeds or errors at torch.cat
                 # Example: return torch.zeros(self.table_feature[next(iter(self.table_feature))].shape[0], self.feature_dict['size'], device=self.device)
                 pass # Let it error on torch.cat if empty, to reveal if this case happens
...
class NodeFeature(nn.Module):
    ...
    def forward(self, features):
        baseline_in, filter_in, table_in, join_in, joinmask_in = torch.split(features, [6, MAXFILTER * 6, 1, MAXJOIN, MAXJOIN], dim = -1)
