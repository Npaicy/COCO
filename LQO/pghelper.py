...
class PGHelper:
    ...
    def get_latency_plan(self, hint, sql, dbName, timeout = PLANMAXTIMEOUT, hintstyle = RULEHINT):
        try:
            if dbName != None and dbName != self.curDbName:
                self.reconnect(dbName)
            self.cur.execute("SET statement_timeout = " + str(int(timeout)) + ";")
            if hintstyle == LEADINGHINT:
    ...
    def get_cost_plan(self, hint, sql, hintStyle, dbName = None):
        ...
        if dbName != None and dbName != self.curDbName:
            self.reconnect(dbName)
        startTime = time.time()
        self.cur.execute("SET statement_timeout = " + str(PLANMAXTIMEOUT) + ";")
        try:
            if hintStyle == LEADINGHINT:
    ...
    def get_result(self, sql, dbName):
        if dbName == 'imdbsyn':
            dbName = 'imdb'
        if dbName != None and dbName != self.curDbName:
            self.reconnect(dbName)
    ...
    def get_statistics(self,old_metaInfo = None):
        def read_str(vals):
            result = []
            current = []
            in_quotes = False
        ...
        for tableName in self.tableNames:
            if old_metaInfo is None:
                ...
                self.tableSize[tableName] = old_metaInfo['tableAttr'][tableName]["table_size"]
        ...
        for table_name in self.tableNames:
            ...
            all_statistics_dict['tableAttr'][table_name]["table_size"] = self.tableSize[table_name]
