...
class DistFeature(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        # Assume input_size is the total size; each bin size is input_size / config.N_bins.
        ...
        self.fit_embed_dim = 32
        self.bin_output_size = self.stat_dim + self.fit_embed_dim
        self.embed = nn.Embedding(3, self.fit_embed_dim)
    ...
    def forward(self, x):
        ...
        concat_embed = concat_embed.view(n_batch, config.max_column_num, n_column, -1)
        return concat_embed
...
class NodeEncoder(nn.Module):
    def __init__(self):
        ...
        self.final_dim = config.model_config['table']['size'] + config.model_config['column']['size'] + self.feature_dict['predicate'] + self.feature_dict['aggregate'][1]
    ...
    def forward(self, x):
        table, column, predicate, aggregate = torch.split(x, [1, 1, 35, 1], dim=-1)
        ...
        combined = torch.cat((table_feat, column_feat, predicate_feat, aggregate_feat), dim=-1)
